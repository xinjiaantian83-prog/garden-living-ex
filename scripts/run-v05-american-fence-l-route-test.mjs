...
function createRouteInstructionImage(sourcePath, outputPath, route) {
  const result = spawnSync('python3', [
    ...
    [
      ...
      'coords = list(map(float, sys.argv[3:9]))',
      'img = Image.open(source).convert("RGBA")',
      'w, h = img.size',
      'points = [(int(coords[i] * w), int(coords[i+1] * h)) for i in range(0, 6, 2)]',
      'overlay = Image.new("RGBA", img.size, (0, 0, 0, 0))',
      'draw = ImageDraw.Draw(overlay)',
      'line_width = max(8, int(min(w, h) * 0.012))',
      'draw.line(points, fill=(210, 42, 31, 220), width=line_width, joint="curve")',
      'for label, point in zip(("START", "CORNER", "END"), points):',
    ...
    String(route.corner.x),
    String(route.corner.y),
    String(route.end.x),
    String(route.end.y),
  ], { encoding: 'utf8' });
  if (result.status !== 0) {
...
function routePrompt(route) {
  return [
    'Version0.5 アメリカンフェンス L字配置指定UI 最小検証用。',
    'アップロードされた元の庭写真をもとに、アメリカンフェンスを施工済みの完成イメージとして生成してください。',
    '別添の指示画像には赤いL字の線とSTART/CORNER/ENDが描かれています。',
    'この赤いL字線をフェンスの設置ルートとして厳守してください。',
    `始点の正規化座標は x=${route.start.x}, y=${route.start.y}、折れ点は x=${route.corner.x}, y=${route.corner.y}、終点は x=${route.end.x}, y=${route.end.y} です。`,
    'フェンスはSTARTからCORNERまでの1辺目と、CORNERからENDまでの2辺目の両方へ配置してください。',
    '折れ点CORNERで2辺のフェンスを自然に連続接続してください。',
    '片方の辺を省略しないでください。',
    '始点、折れ点、終点を極端に短縮しないでください。',
    '指定線以外の場所へフェンスを追加しないでください。',
    '庭の手前へ勝手に移動しないでください。',
    'ゲートや開口部を勝手に追加しないでください。',
    '人工芝、植栽、舗装、タイル、砂利、家具、人、犬、装飾などを追加しないでください。',
    '赤い線、START、CORNER、ENDの文字や目印は最終画像には残さないでください。',
    '元の庭写真の建物、窓、外壁、敷地形状、カメラアングル、地面の状態、縦横比を可能な限り維持してください。',
  ].join(' ');
}
...
function buildPayload(sourcePath, instructionPath) {
  ...
  return {
    scene: '庭',
    concept: '指定L字ルートに沿ったアメリカンフェンス配置検証',
    ...
    reference_products: [
      {
        product_id: 'L-ROUTE-INSTRUCTION-IMAGE',
        product_name: 'フェンスL字設置ルート指示画像',
        category: '配置指示',
        image_url: instructionImageUrl,
        ai_prompt: '赤いL字線をフェンスの設置ルートとして扱う。赤線やSTART/CORNER/END表示は最終画像に残さない。',
      },
...
async function main() {
  ...
  const routeInstructionPath = resolve(OUTPUT_DIR, 'l-route-instruction.jpg');
  copySourceForRoutePicker(sourcePath);
  createRouteInstructionImage(sourcePath, routeInstructionPath, ROUTE);

  const log = {
    version: '0.5-l-route',
    purpose: 'American fence L-route instruction minimal validation. Three points, two line segments.',
  ...
  for (let index = 1; index <= RUN_COUNT; index += 1) {
    ...
    const outputName = `american-fence-l-route-${String(index).padStart(2, '0')}.png`;
